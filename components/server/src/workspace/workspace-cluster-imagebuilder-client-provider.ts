...
export class WorkspaceClusterImagebuilderClientProvider implements ImageBuilderClientProvider {
    ...
    async getDefault(
        ...
        for await (let cluster of clusters) {
            const c = await this.source.getWorkspaceCluster(cluster.installation);
            if (!c) {
                continue;
            }

            var client = this.connectionCache.get(c.name);
            if (!client) {
                client = new ImageBuilderClient(
                    c.url,
                    this.clientProvider.getCredentials(c),
                    this.clientProvider.getGRPCOptions(c, defaultGRPCOptions),
                );
                this.connectionCache.set(c.name, client);
            }
