	...
	initContainers := []corev1.Container{
		{
			...
			Image: common.ImageName(common.ThirdPartyContainerRepo(ctx.Config.Repository, common.DockerRegistryURL), "library/ubuntu", "20.04", &ctx.Config),
			Command: []string{
				"/usr/bin/nsenter",
				"-t",
				"1",
...
`},
		...
		},
		{
			...
			Image: common.ImageName(cfg.Repository, "seccomp-profile-installer", ctx.VersionManifest.Components.WSDaemon.UserNamespaces.SeccompProfileInstaller.Version, &ctx.Config),
			Command: []string{
				"/bin/sh",
				"-c",
		...
		{
			Name:  "sysctl",
			Image: common.ImageName(cfg.Repository, "ws-daemon", ctx.VersionManifest.Components.WSDaemon.Version, &ctx.Config),
			Command: []string{
				"sh",
				"-c",
				`(
...
`,
	...
	if cfg.Workspace.Runtime.FSShiftMethod == config.FSShiftShiftFS {
		initContainers = append(initContainers, corev1.Container{
			...
			Image: common.ImageName(cfg.Repository, "shiftfs-module-loader", ctx.VersionManifest.Components.WSDaemon.UserNamespaces.ShiftFSModuleLoader.Version, &ctx.Config),
			VolumeMounts: []corev1.VolumeMount{{
				Name:      "node-linux-src",
				ReadOnly:  true,
	...
	podSpec := corev1.PodSpec{
		...
		Containers: []corev1.Container{
			{
				Name:  Component,
				Image: common.ImageName(ctx.Config.Repository, Component, ctx.VersionManifest.Components.WSDaemon.Version, &ctx.Config),
				Args: []string{
					"run",
					"--config",
					"/config/config.json",
